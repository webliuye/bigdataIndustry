  ...
  <div class="annual-report">
    <div style="overflow-y: auto; height: 100%;">
      ...
      <div class="operating">
        ...
        <el-button @click="summaryReport">汇总上报</el-button>
    ...
    methods: {
      ...
      getAnnualReportList(pageSize, pageIndex, search) {
        ...
        if (annual.has('streetAnnualReportList').value()) {
          // LocalStorage有
          this.getReportData(annual.get('streetAnnualReportList').value(), pageSize, pageIndex, search)
        }
        // else {
        //   // json文件取
      ...
      passReport(row) {
        ...
        if (row !== 'op') {
          if (+row.status === 4) {
            ...
            this.$message.error('已退回的数据请修改后重新上报')
            flag = false
            return
          } else{
            annual.get('streetAnnualReportList').find(row).assign({status: '4'}).write()
          }
        } else {
          this.selectionData.forEach(thisRow => {
            if (+thisRow.status === 3) {
              ...
              _needList.push(thisRow)
            }
          })
        }        
        // 更新列表
        if (flag) {
          _needList.forEach(thisList => {
            annual.get('streetAnnualReportList').find(thisList).assign({status: '4'}).write()
          })
          this.getAnnualReportList(10, 0)
          this.$message({
            message: '审核通过成功',
      ...
      returnReport(row) {
        ...
        if (row !== 'op') {
          if (+row.status === 4) {
            ...
            annual.get('streetAnnualReportList').find(row).assign({status: '3'}).write()
          }
        } else {
          this.selectionData.forEach(thisRow => {
            annual.get('streetAnnualReportList').find(thisRow).assign({status: '3'}).write()
          })
        }
        // 更新列表
        this.getAnnualReportList(10, 0)
      ...
      summaryReport() {
        const adapter = new LocalStorage('annual')
        const annual = low(adapter)
        let _reportList = annual.get('streetAnnualReportList').value()
        let reportFlag = false
        if (_reportList) {
          if (!annual.has('annualReportList').value()) {
            annual.set('annualReportList', []).write()
          }
          _reportList.forEach(thisReport => {
            if (+thisReport.status === 4 && !thisReport.alreadyReported) {
              const _annualReportList = annual.get('annualReportList').value()
              let flag = false  // 经信是否已存在当前上报的数据
              const arData = []
              _annualReportList.forEach(thisData => {
                if (thisReport.name === thisData.name) {
                  flag = true
                  arData.push(thisData)
                }
              })
              reportFlag = true
              if (!flag) {
                thisReport.alreadyReported = true
                const _needReport = Object.assign({}, thisReport)
                _needReport.status = '2'
                annual.get('annualReportList').push(_needReport).write()
              } else {
                arData.forEach(thisAr => {
                  annual.get('annualReportList').find(thisAr).assign({alreadyReported: true, status: '2'}).write()
                })
              }
            }
          })
          if (reportFlag) {
            ...
            this.$message.error('没有找到可上报的数据')
          }
        } else {
          this.$message.error('没有找到需要上报的数据')
        }
      }
    }
  }
