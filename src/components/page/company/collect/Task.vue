    ...
    <div v-if="!showLook" style="overflow-y: auto; height: 100%;">
      <el-form :inline="true" :model="filterReport" ref="filterReport" class="filter-report" label-width="60px">
        ...
        <el-form-item label="状态：" style="width: 180px">
          <el-select v-model="filterReport.companyReadStatus">
      ...
      <div class="report-list-warp">
        ...
        <el-table
          ...
          <el-table-column
            ...
            label="状态">
            <template slot-scope="scope">
              <span :class="+scope.row.companyReadStatus === 1 ? 'published' : 'pending'">{{+scope.row.companyReadStatus === 1 ? '已阅' : '未阅'}}</span>
            </template>
          </el-table-column>  
          <el-table-column
            label="发布日期"
    ...
    methods: {
      ...
      processData(data, pageSize, pageIndex, search) {
        ...
        data.forEach((thisData, index) => {
          // 已发布的任务才能看
          if (+thisData.status === 1 && thisData.companyChildShow) {
            if (search) {
              // 查询处理
              let _obj = thisData
        ...
        _this.annualReportList.forEach(thisData => {
          ...
          if (+thisData.companyReadStatus === 1) {
      ...
      deleteData() {
        this.selectionData.forEach(thisSelection => {
          ...
          const task = low(adapter)
          if (task.has('taskList').value()) {
            task.get('taskList').find(thisSelection).assign({companyChildShow: false}).write()
          } else {
      ...
      allRead() {
        ...
        if (task.has('taskList').value()) {
          task.get('taskList').value().forEach(thisTask => {
            task.get('taskList').find(thisTask).assign({companyReadStatus: '1'}).write()
          })
        }
        this.getAnnualReportList(10, 0)
      },
      ...
      lookTaskInfo(row) {
        const adapter = new LocalStorage('task')
        const task = low(adapter)
        task.get('taskList').find(row).assign({companyReadStatus: '1'}).write()
        this.showLook = true
