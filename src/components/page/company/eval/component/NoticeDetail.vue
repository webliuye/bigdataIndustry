<template>
  <div>
    ...
    <div
      class="c-fb"
      v-if="(row.eval_status !== '未告知')  && (row.eval_status!=='反馈已退回')"
    >
      <h4>异议反馈</h4>
      <div class="
      fb-con">
    ...
    <!-- <div
      class="c-fb"
      v-if="row.eval_status === '反馈待审核' || row.eval_status === '反馈待已退回'"
    >
      <h4>审核意见</h4>
      <div class="fb-con">
        <div class="item">
          ...
          <p class="right">
            <el-radio-group v-model="isTrueth">
    ...
    <div style="float: left;width: 100%;text-align:right;margin-top:24px;height:60px;">
      <el-button
        type="danger"
        v-if="row.eval_status!=='反馈已退回' && isFb !==  false"
        @click="handleFb()"
      >异议反馈</el-button>
      <el-button
        type="primary"
...
export default {
  ...
  methods: {
    ...
    handleConfirm () {
      ...
      this.row.eval_status = '已确认'
      list.forEach(e => {
        if ((e.comp_name === this.row.comp_name) && (e.year === this.row.year)) {
          e = this.row
        }
      })
