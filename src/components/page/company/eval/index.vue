      ...
      >
        ...
        >
          <template slot-scope="scope">
            <div class="oper-btn">
              <img
                :class="{'ly-gray' : scope.row.eval_status === '已确认'}"
                src="../../../../assets/img/edit.svg"
                @click="(scope.row.eval_status !== '已确认') && handleClick(scope.row)"
                alt=""
              >
              <img
...
export default {
  ...
  data () {
    return {
      ...
      imgurls: { '已告知': getUrl('red'), '已确认': getUrl('green'), '反馈已退回': getUrl('yellow'), '反馈待审核': getUrl('blue') },
  ...
  computed: {
    ...
    tableHeight () {
      return document.body.clientHeight - 248
    }
  },
  created () {
    this.init()
  ...
  methods: {
    init () {
      this.list = eval_db.get('list').value().filter(e => {
        return (e.comp_name === '北京汽车集团有限公司' && e.eval_status !== '未告知')
      })
